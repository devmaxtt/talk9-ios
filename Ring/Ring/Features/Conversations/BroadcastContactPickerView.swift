    ...
    var body: some View {
        NavigationView {
            ...
            .alert("Recipient limit reached",
                   isPresented: $viewModel.limitReached) {
                Button("OK", role: .cancel) { viewModel.limitReached = false }
            } message: {
                Text("You can select up to \(BroadcastViewModel.maxRecipients) contacts.")
            }
            .background(
